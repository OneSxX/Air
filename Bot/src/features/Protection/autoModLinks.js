...
const LINK_AUTOMOD_RULE_NAME = "AIR | Link Koruma";
const LINK_REGEX_PATTERN = String.raw`\b(?:https?:\/\/|www\.|discord(?:app)?\.(?:com\/invite|gg)\/|(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,})(?:[^\s<>()]*)`;
...
async function resolveExistingRule(guild, savedRuleId) {
  ...
  return rules.find(
    (rule) =>
      ...
      rule?.name === LINK_AUTOMOD_RULE_NAME
  ) || null;
}

async function syncLinkAutoModRule(guild, cfg, opts = {}) {
  ...
  try {
    ...
    const enabled = !!cfg?.toggles?.links;
    const timeoutSeconds = normalizeTimeoutSeconds(cfg?.timeouts?.links);
    const allowedLinks = normalizeAllowedLinks(
      linksCfg.allowedLinks || linksCfg.allowList || linksCfg.allowed || []
    );
    const exemptRoles = normalizeSnowflakeList(linksCfg.exemptRoleIds);
    const exemptChannels = normalizeSnowflakeList(linksCfg.exemptChannelIds);
    const savedRuleId = String(linksCfg.autoModRuleId || "").trim();
    ...
    const patch = {
      name: LINK_AUTOMOD_RULE_NAME,
      enabled,
      triggerMetadata: {
        regexPatterns: [LINK_REGEX_PATTERN],
        // Izinli linkler AutoMod tarafinda timeout yemesin.
        // Diger kanallarda yasaklama manuel guard ile devam eder.
        allowList: allowedLinks,
      },
      actions,
      exemptRoles,
      exemptChannels,
      reason: "Protection: Link ayarlarini AutoMod ile senkronla",
    };
