...
} = require("./guards/kufurEngel");

const PROFANITY_AUTOMOD_RULE_NAME = "AIR | Kufur Engeli";
...
async function resolveExistingRule(guild, savedRuleId) {
  ...
  return rules.find(
    (rule) =>
      ...
      rule?.name === PROFANITY_AUTOMOD_RULE_NAME
  ) || null;
}

async function syncProfanityAutoModRule(guild, cfg, opts = {}) {
  ...
  try {
    const profanityCfg = cfg?.profanity || {};
    const enabled = !!cfg?.toggles?.profanity;
    const timeoutSeconds = normalizeTimeoutSeconds(cfg?.timeouts?.profanity);
    const level = normalizeProfanityLevel(profanityCfg.level);
    const keywordFilter = getProfanityAutoModKeywordFilter(level);
    const exemptRoles = normalizeSnowflakeList(profanityCfg.exemptRoleIds);
    const exemptChannels = normalizeSnowflakeList(profanityCfg.exemptChannelIds);
    const savedRuleId = String(profanityCfg.autoModRuleId || "").trim();
    ...
    const patch = {
      name: PROFANITY_AUTOMOD_RULE_NAME,
      enabled,
      triggerMetadata: {
        keywordFilter,
        allowList: [],
      ...
      exemptRoles,
      exemptChannels,
      reason: "Protection: Kufur engel ayarlarini AutoMod ile senkronla",
    };
