...
function buildListEmbed(list) {
  ...
  let description = "Kayitli text level rol odulu yok.";
  ...
  return createEmbed()
    .setTitle("Text Level Rol Listesi")
    .setDescription(description)
    .addFields(
      {
        name: "Silme",
        value: "`/textlevelrol sil text_level:<seviye>`",
        inline: false,
      },
      {
        name: "Sifirlama",
        value: "`/textlevelrol sifirla onay:EVET`",
        inline: false,
      }
    );
}
...
module.exports = {
  name: "textlevelrol",
  description: "Text level rol odullerini ayarlar.",
  async execute(interaction, client) {
    try {
      ...
      const current = Array.isArray(cfg?.textRoleRewards) ? cfg.textRoleRewards : [];
      ...
      if (sub === "sil") {
        const targetLevel = Number(interaction.options?.getInteger?.("text_level", true) || 0);
        if (!Number.isFinite(targetLevel) || targetLevel < 1 || targetLevel > 10_000) {
          return interaction.editReply("Text level 1 ile 10000 arasinda olmali.").catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
        }

        const next = normalizeRewards(current.filter((x) => Number(x?.level) !== targetLevel));
        if (next.length === current.length) {
          return interaction.editReply(`Lv.${targetLevel} icin kayitli text rol odulu yok.`).catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
        }

        await levelFeature.setConfig(client.db, interaction.guildId, {
          textRoleRewards: next,
        });

        return interaction
          .editReply(`Text level rol odulu silindi: **Lv.${targetLevel}**\nKalan kayit: **${next.length}**`)
      ...
      if (sub === "sifirla") {
        ...
        return interaction.editReply("Tum text level rol odulleri sifirlandi.").catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
      }

      if (sub !== "ayarla") {
        return interaction
          .editReply("Gecersiz alt komut. `/textlevelrol ayarla|list|sil|sifirla` kullan.")
          .catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
      }
      ...
      return interaction.editReply(
        `Text level rolu ayarlandi.\n` +
        `- Seviye: **${targetLevel}**\n` +
        `- Rol: <@&${role.id}>\n` +
        `- Toplam kayit: **${next.length}**`
      ).catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
    } catch (err) {
      console.error("textlevelrol command error:", err);
      if (interaction.deferred || interaction.replied) {
        return interaction.editReply("Text level rol ayarlanirken hata olustu.").catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
      }
      return interaction.reply({ content: "Text level rol ayarlanirken hata olustu.", ephemeral: true }).catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
    }
  },
};
