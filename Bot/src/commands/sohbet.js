  ...
  async execute(interaction, client) {
    try {
      ...
      if (sub && sub !== "korumalari") {
        return interaction.editReply("Gecersiz alt komut. /sohbet korumalari kullan.").catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
      }
      ...
      await sendOrUpdatePanel(interaction, cfg, { recreate: true, only: "chat" });

      return interaction.editReply("Sohbet koruma paneli kuruldu / guncellendi.").catch((err) => { globalThis.__airWarnSuppressedError?.(err); });
    } catch (err) {
      console.error("sohbet command error:", err);
      if (interaction.deferred || interaction.replied) {
